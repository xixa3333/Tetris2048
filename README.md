...
- Android：`Tetris2048-Android-v2.3.9.apk`
- Windows：`Tetris2048-Windows-v2.3.9.zip`，解壓縮後執行 `Tetris2048.exe`
...
- 模式1：經典模式，保留 v2.3.8 規則。同色相鄰方塊會合併成同一物件一起移動，策略性較高。
- 模式2：輕鬆模式，保留 v2.3.9 規則。同色相鄰仍保持不同物件，被消除切開後也會變成可獨立移動的物件。
- 兩個模式的動畫、視覺風格與方塊外框一致；排行榜會依模式分開。
...
- 暱稱以 Unicode 字元計算 2～16 字，支援中英文與 Emoji；建立或修改失敗時會顯示登入、權限、格式或網路原因。
