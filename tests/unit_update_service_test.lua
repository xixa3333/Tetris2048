...
T.test("Update checker compares semantic release versions",function()
    T.equal(UpdateService.isNewer("v2.3.8","2.3.7"),true)
...
T.test("Update checker returns only the configured trusted download URL",function()
    ...
    function http:request(method,url,body,headers,callback)
        ...
        callback(true,{tag_name="v2.3.8",html_url="https://example.com/phishing"})
    end
    local service=UpdateService.new(http,{currentVersion="2.3.7",latestReleaseApiUrl="https://api.github.test/latest",
        latestReleaseUrl="https://github.com/xixa3333/Tetris2048/releases/latest"})
    service:check(function(ok,result)
        T.equal(ok,true); T.equal(result.updateAvailable,true); T.equal(result.version,"2.3.8")
