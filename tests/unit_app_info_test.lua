...
T.test("APP information contains safe links and every released version summary",function()
    ...
    T.equal(info.currentVersion,"2.4.0")
    T.equal(info.displayName,"BlockMerge 2048")
    T.equal(info.latestReleaseUrl,"https://github.com/xixa3333/Tetris2048/releases/latest")
    T.equal(#info.versions,18)
    T.equal(info.versions[1].version,"2.4.0")
    T.equal(info.versions[2].version,"2.3.9")
